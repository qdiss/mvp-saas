...
import { DollarSign, Package, Star, FileImage } from "lucide-react"
...
export function StatsCards({ myProduct, competitors }: StatsCardsProps) {
    ...
    const avgRating = selectedCompetitors.length > 0
        ? (selectedCompetitors.reduce((sum, c) => sum + c.rating, 0) / selectedCompetitors.length).toFixed(1)
        : "0.0"
    
    const avgImages = selectedCompetitors.length > 0
        ? (selectedCompetitors.reduce((sum, c) => sum + c.photos.length, 0) / selectedCompetitors.length).toFixed(1)
        : "0.0"
    
    const sponsoredCount = selectedCompetitors.filter(c => c.isSponsored).length
    ...
    return (
        <div className="grid grid-cols-5 gap-3">
            ...
            <StatCard
                label="Images"
                value={myProduct.photos.length.toString()}
...
function StatCard({ label, value, subtitle, icon, colorClass }: StatCardProps) {
    ...
    return (
        <div className={`bg-gradient-to-br ${c.bg} rounded-lg p-3 border ${c.border}`}>
