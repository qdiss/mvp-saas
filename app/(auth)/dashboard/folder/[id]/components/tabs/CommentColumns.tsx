// app/folders/[id]/components/tabs/CommentsColumn.tsx
"use client";
      ...
      {selectedImageIds.length > 0 && (
        <div className="mb-3 p-3 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200">
          ...
          <div className="flex gap-1.5 flex-wrap">
            {selectedImageIds.slice(0, 6).map((imageId) => (
              <img
                key={imageId}
                src={getPhotoUrl(imageId)}
                alt="Selected"
                className="w-12 h-12 object-cover rounded-lg border-2 border-blue-400 shadow-sm"
              />
            ))}
            {selectedImageIds.length > 6 && (
              <div className="w-12 h-12 rounded-lg border-2 border-dashed border-blue-300 bg-blue-50 flex items-center justify-center">
        ...
        <div className="flex-1 overflow-auto space-y-3 mb-4 min-h-[200px] max-h-[600px]">
          {comments.length === 0 ? (
            <div className="text-center py-12 text-slate-500">
              ...
              <p className="text-xs mt-1">Select images to add comments</p>
            </div>
          ) : (
                    ...
                    {comment.images.map((imageUrl, idx) => (
                      <div key={idx} className="relative group">
                        <img
                          ...
                          className="w-full aspect-square object-cover rounded-lg border border-slate-200 group-hover:border-slate-300 transition-colors"
                        />
                      </div>
                    ))}
                  </div>
      ...
      <div className="border-t-2 border-slate-200 pt-4">
        {selectedImageIds.length > 0 && (
          <div className="mb-3 px-3 py-2 bg-emerald-50 border border-emerald-200 rounded-lg">
            <span className="text-xs font-semibold text-emerald-700">
              Commenting on {selectedImageIds.length} image
        ...
        >
          {savingComment ? (
            ...
            <>
              <MessageCircle className="h-4 w-4" />
              {selectedImageIds.length > 0 ? "Post Comment" : "Select Images"}
            </>
          )}
        </button>
      </div>
